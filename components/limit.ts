import { getRedis, RedisClientType } from "@/components/cache";
...
const limitValue = async (
  ...
  if (increment === 0) {
    ...
    const replies = await redis
      ...
      .exec();
    const value = (replies as number[])[0] as number;
...
export const timestampDay = (now: Date) =>
  new Date(
    now.getTime() -
...
const timestampMonth = (now: Date) =>
  new Date(
    now.getTime() -
      ...
      now.getUTCMinutes() * 60 * 1000 -
      now.getUTCHours() * 3600 * 1000 -
      (now.getUTCDate() + 1) * 3600 * 1000 * 24
  ).toISOString();

const timestampHour = (now: Date) =>
  new Date(
    now.getTime() - now.getUTCMilliseconds() - now.getUTCSeconds() * 1000 - now.getUTCMinutes() * 60 * 1000
  ).toISOString();

const timestampMinute = (now: Date) =>
  new Date(now.getTime() - now.getUTCMilliseconds() - now.getUTCSeconds() * 1000).toISOString();

const timestampSecond = (now: Date) => new Date(now.getTime() - now.getUTCMilliseconds()).toISOString();
...
export const Limit = {
   ...
   **/
  month(prefix: string, max: number) {
  ...
  day(prefix: string, max: number) {
    ...
    return (increment = 1) => limitValue(redis, prefix, increment, max, 3600 * 24, timestampDay);
  },
  ...
  hourUnique(prefix: string, max: number) {
    ...
    return (key?: string, fetchKeys?: boolean) => limitUnique(redis, prefix, max, 3600, timestampHour, key, fetchKeys);
  },
  ...
  minuteUnique(prefix: string, max: number) {
    ...
    return (key?: string, fetchKeys?: boolean) => limitUnique(redis, prefix, max, 60, timestampMinute, key, fetchKeys);
  },
};
